...
document.addEventListener('DOMContentLoaded', () => {
    ...
    audioBtn.title = "Arka Plan Sesi (Aç/Kapat)";
    ...
    function renameHeaders(container) {
        // Reset "Seçenek A/B/C" headers if present
        const options = ["Seçenek A", "Seçenek B", "Seçenek C", "Seçenek D", "Seçenek E"];
        ...
        headers.forEach(header => {
            ...
            // i.e. contains "Seçenek" or matches pattern
            if (header.innerText.includes('Seçenek')) {
                // Preserve the rest of the text? "Seçenek A: Tam Şeffaflık" -> "Seçenek A: Tam Şeffaflık" is pointless
                // We need to replace "Seçenek [Old]" with "Seçenek [New]"
                header.innerHTML = header.innerHTML.replace(/Seçenek [A-Z]/, options[labelIndex]);
                labelIndex++;
            }
        });
    }
